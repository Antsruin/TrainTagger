...
// ── 2D Dot: out = A · X, shapes (N,N) x (N,F) → (N,F) ──────────────────────
struct dot2d_config {
    static const unsigned n_rows  = 16;
    static const unsigned n_cols  = 16;
    static const unsigned n_feats = 20;
    typedef ap_fixed<28, 16, AP_RND, AP_SAT, 0> accum_t;   // wider than output to absorb n_cols=16 accumulations
                                         // rule: output<24,12> + log2(16)=4 bits → <28,16>
};
...
template <class input1_T, class input2_T, class res_T, typename CONFIG_T>
void dot2d(
    input1_T A  [CONFIG_T::n_rows  * CONFIG_T::n_cols ],   // flattened (N, N)
    input2_T X  [CONFIG_T::n_cols  * CONFIG_T::n_feats],   // flattened (N, F)
    res_T  out[CONFIG_T::n_rows  * CONFIG_T::n_feats]    // flattened (N, F)
) {
    #pragma HLS PIPELINE
    ...
    for (int n = 0; n < CONFIG_T::n_rows; n++) {
        #pragma HLS UNROLL
        for (int f = 0; f < CONFIG_T::n_feats; f++) {
            #pragma HLS UNROLL
             // Use wider accum_t to safely accumulate n_cols products
            // without overflow before casting down to res_T at the end
            typename CONFIG_T::accum_t acc = 0;

            for (int k = 0; k < CONFIG_T::n_cols; k++) {
                #pragma HLS UNROLL
                // cast product to accum_t before adding — prevents intermediate truncation
                // Cast inputs BEFORE multiplying, not after
                acc += (typename CONFIG_T::accum_t)A[n * CONFIG_T::n_cols + k] 
                    * (typename CONFIG_T::accum_t)X[k * CONFIG_T::n_feats + f];
            }

            // Cast down to output precision only ONCE, after full accumulation
            out[n * CONFIG_T::n_feats + f] = (res_T)acc;
        }
    }
}
