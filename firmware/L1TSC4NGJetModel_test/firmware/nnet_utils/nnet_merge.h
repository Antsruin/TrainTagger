...
struct dot2d_config {
    static const unsigned n_rows = 16;   // N
    static const unsigned n_cols = 16;   // N (inner dim)
    static const unsigned n_feats = 20;  // F
};
...
void dot2d(
    data_T A  [CONFIG_T::n_rows  * CONFIG_T::n_cols ],   // flattened (N, N)
    data_T X  [CONFIG_T::n_cols  * CONFIG_T::n_feats],   // flattened (N, F)
    ...
    for (int n = 0; n < CONFIG_T::n_rows; n++) {
        ...
        for (int f = 0; f < CONFIG_T::n_feats; f++) {
            ...
            for (int k = 0; k < CONFIG_T::n_cols; k++) {
                #pragma HLS UNROLL
                acc += A[n * CONFIG_T::n_cols + k] * X[k * CONFIG_T::n_feats + f];
            }
            out[n * CONFIG_T::n_feats + f] = acc;
        }
    }
}
